...
var resizePizzas = function(size) {
  ...
  function changeSliderLabel(size) {
    switch(size) {
      ...
      case "3":
        ...
        return;
      default:
        window.console.log("bug in changeSliderLabel");
    }
  }
    ...
    function sizeSwitcher (size) {
      switch(size) {
        ...
        case "3":
          return 50;
        default:
          window.console.log("bug in sizeSwitcher");
      }
    }

    var newwidth = sizeSwitcher(size);
    var container = document.getElementsByClassName('randomPizzaContainer'); //getting element by class name
  ...
  window.console.log("Time to resize pizzas: " + timeToResize[timeToResize.length-1].duration + "ms");
};

window.performance.mark("mark_start_generating"); // collect timing data

// This for-loop actually creates and appends all of the pizzas when the page loads
var pizzasDiv = document.getElementById("randomPizzas"); //moved out of the loop
for (var i = 2; i < 100; i++) {
...
window.console.log("Time to generate pizzas on load: " + timeToGenerate[0].duration + "ms");
...
function logAverageFrame(times) {   // times is the array of User Timing measurements from updatePositions()
  ...
  window.console.log("Average scripting time to generate last 10 frames: " + sum / 10 + "ms");
}
...
function updatePositions() {
  ...
  var phase = [];
  for (var j = 0; j < 5; j++) {
    phase.push(Math.sin(cachedScrollTop / 1250 + j) * 100);
}
for (var i = 0; i < len; i++) {
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  var numPizza = window.innerHeight/s * cols; //calculates number of pizza based on the window height
